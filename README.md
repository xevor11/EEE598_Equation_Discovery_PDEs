...
In the report, SINDy and transformer-based symbolic regression frameworks were discussed in the context of the MDBench ODE discovery ablation studies; however, these frameworks were not implemented directly in this work. Instead, the project focused on implementing and analyzing PINN, Symbolic Regression (genetic programming-based), and PDE-FIND frameworks, as these approaches demonstrated strong performance and interpretability across existing literature for nonlinear PDE discovery problems such as Burgers’ and Kuramoto–Sivashinsky equations. In particular, PDE-FIND was selected due to its established sparse regression formulation for PDE identification, while PINN-based equation discovery frameworks have shown robustness under noisy and limited-data regimes. The symbolic regression implementation was inspired by existing genetic equation discovery repositories and evolutionary symbolic regression approaches. Transformer-based methods and SINDy frameworks were instead referenced through MDBench benchmark experiments and related open-source implementations due to the significant computational cost, pretraining requirements, and engineering complexity associated with reproducing large-scale transformer architectures and extensive ODE benchmark pipelines from scratch
